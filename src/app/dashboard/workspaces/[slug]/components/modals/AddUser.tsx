...
import { sendWorkspaceInvitationService } from "@/services/client/workspace.service";
...
const AddUserModal = () => {
    ...
    const { workspaceMembers, fetchWorkspaceMembers } = useWorkspaceMemberStore();
    ...
    const handleClick = async () => {
        if (email && email.includes("@")) {
            try {
                setIsLoading(true);
                await sendWorkspaceInvitationService({
                    email: email,
                ...
                if (apiErrors && typeof apiErrors === "object") {
                    if (apiErrors['non_field_errors']) {
                        ...
                        showToast("İstifadəçi mövcuddur və ya artıq dəvət göndərilib", ToastType.ERROR);
                        setIsLoading(false);
                    }
                }
            }
    ...
    return (
        <>
            <Button onPress={onOpen} className="bg-primary w-full md:w-[142px] rounded-[8px] text-white text-[14px] font-medium">
                <Plus />
                <span className="whitespace-nowrap">Üzv əlavə et</span>
            </Button>
            <Modal.Modal
                size="2xl"
                isOpen={isOpen}
                        ...
                        <Modal.ModalBody>
                                ...
                                <div className="mt-[8px] flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-2">
                                    ...
                                    <div className="flex w-full items-center gap-2">
                                        {memberShipStatuses && memberShipStatuses.length > 0 && (
                                            <Form.Select
                                                placeholder="Status"
                                                onValueChange={(e) => setStatus(String(e))}
                                                value={status}
                                        ...
                                        <button disabled={isLoading} onClick={handleClick} type="button" className="whitespace-nowrap flex gap-[8px] justify-center place-items-center h-[48px] bg-primary px-[20px] rounded-[12px] min-w-[102px] text-white">
                            ...
                            <div className="mt-[24] mb-[10px]">
                                <div className="border-b border-border-gray pb-[6px]">
                                    <span className="text-[14px] pb-[9px] border-b border-primary text-t-black font-medium">Üzv sayı ({workspace?.members_count})</span>
                                </div>
                                <div className="mt-[24px] grid gap-[8px]">
                                    {workspaceMembers.filter((item) => item.status.key === "accepted").map((item, index) => item.user && (
                                        <div
                                            key={index}
