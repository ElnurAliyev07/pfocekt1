...
const InvantionCard: React.FC<Props> = ({invitation}) => {
    ...
    const handleAction = async (action: 'accept' | 'reject') => {
      if (action === 'accept') {
        if (user){
          try{
            await acceptOrRejectProjectInvitationService(invitation.token, {status: 'accepted'});
    ...
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="bg-gray-100 p-8 rounded-lg shadow-lg max-w-md text-center">
        <h1 className="text-3xl font-semibold text-gray-900 mb-4">
          Siz `{invitation.project.title}` adlı proyektə `{invitation.invited_by.full_name}` tərəfindən dəvət olunmusunuz!
        </h1>
